...
def load_data() -> pd.DataFrame:
    """Load the rucu dataset from the data/raw folder."""
...
def make_clean_view(df: pd.DataFrame) -> pd.DataFrame:
    ...
    #------------Create Grade column if missing---------
    if "Grade" not in df.columns:
        LOG.warning("Grade column not found. Creating letter grades from numeric scores")
...
def train_model(df_clean: pd.DataFrame) -> RandomForestClassifier:
...
def predict_example(model: RandomForestClassifier) -> None:
    ...
    predicted_grade: str = model.predict(new_case)[0]
    LOG.info(f"Predicted grade for new case: {predicted_grade}")

# === Section 8. Create Visualizations ===


def make_plots(df_clean: pd.DataFrame, model: RandomForestClassifier) -> None:
    """Create charts for the supervised classification case."""
    LOG.info("Creating chart: hours studied vs grade")
    ...
    scatter_plt.set_title("Hours Studied vs Grade (CLOSE chart to continue)")
    scatter_plt.set_xlabel("Hours Studied")
    scatter_plt.set_ylabel("Grade")
    ...
    bar_plt.set_title("Model Importances (CLOSE chart to continue)")
